...
describe('medalFor (REV-80)', () => {
  it('above 90 is gold, 80 to 90 silver, below 80 bronze', () => {
    ...
    expect(medalFor(0, 100)).toBe('bronze');
  });
  ...
  it('a shorter target is judged by its share of the maximum', () => {
    ...
    expect(medalFor(39, 50)).toBe('bronze');
    expect(medalFor(5, 0)).toBe('bronze');
  });
});
...
describe('the marks (REV-79, REV-80)', () => {
  it('the star carries its medal and the score inside it', () => {
    ...
    expect(renderScoreStar(0, 0, 60, 100)).toContain('data-medal="bronze"');
  });
