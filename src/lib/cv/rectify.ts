...
export function rectify(
  ...
  const side = rectifiedSidePx(template);
  const geom: RectifiedGeometry = { side, pxPerMm: CANONICAL_PX_PER_MM };
  ...
  try {
    ...
    return { gray: out, valid, side, pxPerMm: CANONICAL_PX_PER_MM };
  } catch (err) {
    out.delete();
    valid.delete();
    throw err;
