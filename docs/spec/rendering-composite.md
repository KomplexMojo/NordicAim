...
- **Precision cell (top-left, REV-86)**: the text chip is replaced by a solid-black pictogram (a round head, a broad body, raised arms and a long rifle) of the shooter `standing` or `prone` on a white rounded panel, about 168 by 106 px at (10, 8); a blank precision slot shows `prone` (first) then `standing` (second). A stored `both` target keeps its text chip.
