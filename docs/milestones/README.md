...
| [M19](M19-backing-sheet.md) | Coloured backing sheet option | add metadata (option) · generate analysis | M16 | opus · high | opus · high | no | blocked: M19 Open question 1 (BLOCKING, owner-only): `Auto` — the default mode, which the v1->v2 migration sets on every existing session — needs the owner's answer on which reference photos actually had a backing sheet |
...
| [M15](M15-mvp-release.md) | Install, offline, polish, MVP release | release | M13, M14, M16, M17, M18, M19, M20 | sonnet · medium | sonnet · high | yes | pending |
